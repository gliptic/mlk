...
var isArray = typeof Array.isArray === 'function' ? Array.isArray : function (obj) {
    return toString_.call(obj) == '[object Array]';
};
...
export function reduce(coll, xform, init) {
    if (isArray(coll)) {
        var result = init;
        var index = -1;
        var len = coll.length;
...
export function transduce(coll, xform, reducer, init) {
    xform = xform(reducer);
    if (init === undefined) {
        init = xform.init();
    }
...
export function bound(f, ctx, count?: number) {
...
export function dedupe(coll = undefined) {
    if (coll) {
...
export function takeWhile(f, ctx?);
...
export function take(coll, n: number = undefined) {
...
export function drop(coll, n = undefined) {
    if (isNumber(coll)) { n = coll; coll = null }
...
export function dropWhile(f, ctx?);
...
export function partition(n: number);
export function partition(coll, n?: number) {
    if (isNumber(coll)) {
        n = coll; coll = null;
    }
...
export function partitionBy(f, ctx?);
...
export function toArray(coll, xform) {
    if (!xform) {
...
export function toObj(coll, xform) {
    if (!xform) {
...
export function toIter(coll, xform) {
    if (!xform) {
...
export function LazyTransformer(xform, coll) {
